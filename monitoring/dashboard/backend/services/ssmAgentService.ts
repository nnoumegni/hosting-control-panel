...
import type { ServerSettingsInternal } from '../config/server-settings-provider.js';
        ...
        if (status === 'Success') {
          ...
          try {
            ...
            try {
              ...
              const jsonrepair = jsonrepairModule.jsonrepair || jsonrepairModule.default;
            ...
            if (errorPosition > 0 && errorPosition < jsonString.length) {
              ...
              const contextText = jsonString.substring(start, end);
...
// Note: ssmAgentService will be initialized in index.ts after MongoDB connection
